...
class KaffPlatformSystem {
			...
			case 1: {
				...
				do {
					if (lvl>=1 && lvl<=10)
						...
						{System.out.print("Level must be between 0-10.. Enter level again:");
						lvl = input.nextInt();
						}
				}while(true);
				...
				do {
					if (prc>=0 && prc<=500)
						...
						{System.out.print("Book Price must be between 0-500 SR.. Enter price again:");
						prc = input.nextInt();
						}
				}while(true);
				...
				String s = input.next();
				do {
					if (s.equalsIgnoreCase("sale") || s.equalsIgnoreCase("borrow"))
						break;
					else
						{System.out.print("Please enter either (sale/borrow)");
						s = input.next();
						}
				}while(true);
				...
				if (shelve.addBook(b)) 
					...
					System.out.println("Book was not added successfuly, it seems there's a kind of error.");
	...
	// to add books before displaying the lists..
		static public void fillShelve() {
			...
			Person admin = new Owner("Admin", "11111", 0, "05050505", "admin@kaff.com", true, true, 0);
			Person admin2 = new Owner("Admin2", "11122", 0, "05050505", "admin@kaff.com" , true, true, 0);
			user.add(admin);
			user.add(admin2);
			shelve.addBook(new Book("Introduction to Computing", "CS101", 3, admin, true, 200, "Sale"));
			shelve.addBook(new Book("Network", "CIS315", 5, admin, true, 200, "Sale"));
			shelve.addBook(new Book("Logic and proof", "Math314", 7, admin, true, 0, "Borrow"));
			shelve.addBook(new Book("Operating Systems", "CS999", 9, admin, true, 0, "Borrow"));
			shelve.addBook(new Book("Discrete Mathematics", "Math313", 5, admin, true, 400, "Sale"));
